...
public interface QuoteRepository extends JpaRepository<Quote, Integer> {
    @Query(value = "SELECT * FROM get_quotes_by_authors_id_and_tags_id(:tagsId, :authorsId, :startIndex, 10)", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM get_user_added_quotes_by_user_id(:userId, :startIndex, 10)", nativeQuery = true)
    List<Quote> getSavedUserQuotes(@Param("userId") Integer userId, @Param("startIndex") Integer startIndex);

    @Query(value = "SELECT * FROM get_random_user_not_added_quotes_by_user_id(:userId, 10)", nativeQuery = true)
